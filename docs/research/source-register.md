...
| Mount the client application through one checked React root and enable development diagnostics | [React: `createRoot`](https://react.dev/reference/react-dom/client/createRoot) and [`StrictMode`](https://react.dev/reference/react/StrictMode) | React documents a single client root for a fully React application and recommends Strict Mode for newly created applications. |
| Establish semantic workspace regions and a first-focus bypass | [W3C: semantic HTML regions](https://www.w3.org/WAI/WCAG21/Techniques/html/H101.html) and [Skip Link Easy Check](https://www.w3.org/WAI/test-evaluate/easy-checks/skip-link/) | W3C documents landmarks as the programmatic page structure and a visible-on-focus skip link as a direct path past repeated navigation. |
| Test the shell through the accessibility tree | [Testing Library: query priority](https://testing-library.com/docs/queries/about#priority) | Testing Library prioritizes role, accessible name, label, and visible text queries that reflect user and assistive-technology behavior. |
| Store scientific dimensions in coherent SI units | [BIPM: SI Brochure](https://www.bipm.org/en/publications/si-brochure) and [SI base units](https://www.bipm.org/en/measurement-units/si-base-units) | BIPM defines metre and second as SI base units and area/volume/flow-related quantities as coherent derived units built from them. |
